...
function GymVisual({ est }: { est: string }) {
  return (
    <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-white/5 bg-gradient-to-br from-dark-card to-dark">
      {/* Gradient accents */}
      <div className="absolute -right-20 -top-20 h-40 w-40 rounded-full bg-primary/10 blur-[50px] sm:h-64 sm:w-64 sm:blur-[80px]" />
      <div className="absolute -bottom-10 -left-10 h-28 w-28 rounded-full bg-primary/5 blur-[40px] sm:h-40 sm:w-40 sm:blur-[60px]" />
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <motion.div
          className="h-36 w-36 rounded-full border border-primary/10 sm:h-52 sm:w-52"
          animate={{ scale: [1, 1.2, 1], opacity: [0.2, 0.4, 0.2] }}
          transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute h-52 w-52 rounded-full border border-primary/5 sm:h-72 sm:w-72"
      ...
      >
        <svg className="h-5 w-5 text-primary/40 sm:h-8 sm:w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      >
        <svg className="h-4 w-4 text-primary/40 sm:h-7 sm:w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
